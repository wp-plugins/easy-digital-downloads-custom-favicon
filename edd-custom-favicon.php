...
if( !defined( 'EDD_CUSTOM_FAVICON_VERSION' ) ) {
	define( 'EDD_CUSTOM_FAVICON_VERSION', '1.0.0' ); //version of plugin
}
...
function edd_custom_favicon_load() {
	if( !class_exists( 'Easy_Digital_Downloads' ) ) {
		function edd_custom_favicon_notice() {
		    ...
		    <div class="error">
		        <p>
		        	<strong><?php _e( 'Its seems to be Easy Digital Downloads is not activated. Please first active Easy digital downloads. If you don\'t have Easy Digital Downloads then', 'eddcustomfavicon' ); ?></strong>
		        	<a href="https://wordpress.org/plugins/easy-digital-downloads/" target="_blank"><?php _e('CLICK HERE'); ?></a>
		        	<strong><?php _e('to download', 'eddcustomfavicon'); ?></strong>
		        </p>
		    </div>
		    <?php
		}
		...
		function edd_custom_favicon_add_settings_link( $links, $file ) {
			...
			if ( $file == $this_plugin ) {
				$settings_link = '<a href="edit.php?post_type=download&page=edd-settings&tab=extensions">' . __( 'Settings', 'eddcustomfavicon' ) . '</a>';
		...
		$edd_custom_favicon_public = new Edd_Custom_Favicon_PublicPages();
		...
		$edd_custom_favicon_admin = new Edd_Custom_Favicon_AdminPages();
